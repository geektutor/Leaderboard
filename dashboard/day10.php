<?php
  ...
  function getUSerRankings($fetched_array)
  {
    ...
    if (isset($_GET['filter']) && !empty($_GET['filter'])) {
      ...
      switch ($filter) {
        case 'overall':
          $sql = "SELECT * FROM user10 WHERE `isAdmin` = 0 ORDER BY `score` DESC LIMIT 20";
          break;
        
        default:
          $sql = "SELECT * FROM user10 WHERE `isAdmin` = 0 AND `track` ='$filter' ORDER BY `score` DESC LIMIT 20";
          break;
      }
    }else{
      $sql = "SELECT * FROM user10 WHERE `isAdmin` = 0 ORDER BY `score` DESC LIMIT 20";
