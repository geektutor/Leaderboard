...
<body>
    ...
    <?php
            ...
            if(isset($_POST['submit'])){
                ...
                $checkIt = check($email);
                if($checkIt){
                    $sql = "INSERT INTO user(`first_name`, `last_name`, `user_id`, `nickname`, `email`, `password`, `phone`) 
                            VALUES('$fname', '$lname', '$user_id', '$nick', '$email', '$password', '$phone')";
                    if($conn->query($sql)){
        ...
        <form class="signup" action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']);?>" method="POST">
            <h2>Sign Up</h2>
            <div class="group"><input type="tetx" placeholder="First name" name="fname" required><i class="fa fa-use"></i></div>
            <div class="group"><input type="text" placeholder="Last name" name="lname" required><i class="fa fa-user"></i></div>
